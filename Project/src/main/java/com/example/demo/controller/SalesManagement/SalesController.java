...
public class SalesController {
	...
	public String newInstallationCertificate(@PathVariable int id, ModelMap model) {
		InstallationCertificate installationcertificate = installationcertificatedao	.getAllInstallationCertificateByID(id);
	...
	@RequestMapping(value = "/editins/{id}", method = RequestMethod.GET)
	...
	public ModelAndView getRange(@Valid @ModelAttribute("range") ClientOrder clientorder, BindingResult result,
			@RequestParam("range_1") String range_1, @RequestParam("range_2") String range_2) {
		// if(result.hasErrors()) {
		// return "ViewClientOrder2";
		// }
		List<ClientOrder> list = clientorderdao.getAllCleintOrdersByRange(range_1, range_2);
...
public String salesDashBord() {
	return "SalesManagement/SalesDashBord";
}
