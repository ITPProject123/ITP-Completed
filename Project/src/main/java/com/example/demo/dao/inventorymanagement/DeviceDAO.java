...
JdbcTemplate template;
	...
	public void save(Device d) {
		String sql = "insert into device(deviceID, model, unitPrice, name, manufacturer,suppID) values ('"+d.getDeviceID()+"','"+d.getModel()+"','"+d.getUnitPrice()+"','"+d.getName()+"','"+d.getManufacturer()+"')";
	...
	public void update(Device d) {
		String sql = "update device set name='"+d.getName()+"', unitPrice='"+d.getUnitPrice()+"' where deviceID="+d.getDeviceID()+"";
		System.out.println(sql);
		template.update(sql);
	}
