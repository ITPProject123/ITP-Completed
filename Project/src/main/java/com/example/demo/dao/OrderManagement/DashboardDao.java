...
JdbcTemplate template;
	...
	public List<ClientOrder> Dashboard2() {
		...
		return template.query("SELECT * FROM client_order inner join client_order_device on client_order.clieOrdID=client_order_device.clOrderID ORDER BY clieOrdID DESC LIMIT 5",new ResultSetExtractor<List<ClientOrder>>(){
			...
			while(rs.next()) {
				...
				d.setItem(rs.getInt(9));
				d.setQuantity(rs.getInt(11));
