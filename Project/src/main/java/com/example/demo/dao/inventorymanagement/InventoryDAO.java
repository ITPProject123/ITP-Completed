...
public class InventoryDAO {
	...
	public List<Inventory> getAllInventory(){
		return template.query("select * from inventory", new ResultSetExtractor<List<Inventory>>() {
			public List<com.example.demo.model.inventorymanagement.Inventory> extractData(ResultSet rs) throws SQLException, DataAccessException
			{
	...
	public Inventory getInventoryById(int devid){
		return template.query("select * from inventory where deviceID="+devid, new ResultSetExtractor<Inventory>() {
			public com.example.demo.model.inventorymanagement.Inventory extractData(ResultSet rs) throws SQLException, DataAccessException
			{
				Inventory inv = new Inventory();
				while(rs.next()) {
	...
	public void update(Inventory i) {
		String sql = "update Inventory set quantity='"+i.getQuantity()+"', criticalLevel='"+i.getCriticalLevel()+"' where deviceID="+i.getDeviceID()+"";
		template.update(sql);
	}
