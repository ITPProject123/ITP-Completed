...
import com.example.demo.model.SalesManagement.Employee;
...
//Generate Total Amount by id
	public void generateAmountByID(int id) {
			...
			String sql = "SET @VAR = (select SUM(id.quantity * d.unitPrice) from installation_device id,device d ,sales s,client_order co,installation_certificate ic where id.devicesCode = d.deviceCode AND s.orID= co.client_orderCode AND ic.ordId=co.client_orderCode AND id.instID=ic.installationID AND s.salesID="+id+");"; 
					 
			 String sql2 = "update sales SET totalAmount = @var  where salesID= "+id+" ";
			
	
			System.out.println("Generate Total Amount sql1 " + sql);
			System.out.println("Generate Total Amount sql2 " + sql);
			template.batchUpdate(sql,sql2);

		}	
		
//Update status copmleted by id
	public void updateStatusCompletedByID(int id) {
			...
			System.out.println("Update status completed" + sql);
...
//Update status Pending by id
		public void updateStatusPendingByID(int id) {
				...
				System.out.println("Update status pending" + sql);
...
//Delete Sales details By ID
		...
		public void delete(int id) {
			...
			String sql = "delete from sales where salesID=" + id + "";
